/*
  ...
  - You are about to alter the column `statut` on the `demandeintervention` table. The data in that column could be lost. The data in that column will be cast from `VarChar(191)` to `Enum(EnumId(0))`.
...
-- DropIndex
DROP INDEX `Contrat_idSite_fkey` ON `contrat`;
...
-- AlterTable
ALTER TABLE `demandeintervention` MODIFY `statut` ENUM('NON_PLANIFIE', 'PLANIFIE') NOT NULL DEFAULT 'NON_PLANIFIE';

-- AlterTable
ALTER TABLE `intervention` ADD COLUMN `statut` ENUM('PROGRAMME', 'EN_COURS', 'SUSPENDU', 'TERMINE') NOT NULL DEFAULT 'PROGRAMME';

/*
  ...
  - You are about to drop the column `dateDebutContrat` on the `contrat` table. All the data in the column will be lost.
  - You are about to drop the column `dateFinContrat` on the `contrat` table. All the data in the column will be lost.
  - You are about to drop the column `idInstallationEq` on the `contrat` table. All the data in the column will be lost.
  ...
  - Added the required column `idSite` to the `Contrat` table without a default value. This is not possible if the table is not empty.
  - Added the required column `nom` to the `Contrat` table without a default value. This is not possible if the table is not empty.
  - Added the required column `termeContrat` to the `Contrat` table without a default value. This is not possible if the table is not empty.
...
-- AlterTable
ALTER TABLE `contrat` DROP COLUMN `dateDebutContrat`,
    DROP COLUMN `dateFinContrat`,
    DROP COLUMN `idInstallationEq`,
    ...
    ADD COLUMN `termeContrat` VARCHAR(191) NOT NULL,
    ADD COLUMN `typeContrat` VARCHAR(191) NOT NULL;

-- AlterTable
ALTER TABLE `utilisateur` ADD COLUMN `etatCompte` VARCHAR(191) NOT NULL DEFAULT 'actif';
...
-- AddForeignKey
ALTER TABLE `Contrat` ADD CONSTRAINT `Contrat_idSite_fkey` FOREIGN KEY (`idSite`) REFERENCES `Site`(`id`) ON DELETE RESTRICT ON UPDATE CASCADE;
